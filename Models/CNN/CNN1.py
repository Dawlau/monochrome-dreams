...
def define_model(): # create the network
	...
	model.add(Conv2D(1650, (5, 5), activation = 'relu', input_shape = (32, 32, 1))) # add the first convolutional layer
	...
	model.add(Dense(1000, activation = 'relu')) # dense layer
	model.add(Dense(9, activation = 'softmax')) # each node will have the probability for the respective class

	print(model.summary()) # show the structure of the network

	opt = tf.keras.optimizers.SGD(lr = 0.01, momentum = 0.9) # create optimizer
