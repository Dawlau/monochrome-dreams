...
def define_model(): # create network

	from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Dropout

	model = tf.keras.Sequential()

	model.add(Conv2D(500, (5, 5), activation = 'relu', input_shape = (32, 32, 1))) # create the first convolutional layer that takes in the input
	model.add(Conv2D(32, (3, 3), activation='relu')) # second conv layer

	model.add(MaxPooling2D(pool_size=(2, 2))) # add a pooling layer to get the best features of the images
	model.add(Dropout(0.25)) # ignore some neurons

	model.add(Conv2D(64, (3, 3), padding='same', activation='relu')) # conv layer
	...
	model.add(Dropout(0.5)) # ignore some neurons
	model.add(Dense(9, activation = 'softmax')) # each node will have the probability for the respective class

	print(model.summary())

	opt = tf.keras.optimizers.SGD(lr = 0.01, momentum = 0.9)
	model.compile(optimizer = opt, loss = 'categorical_crossentropy', metrics = ['accuracy']) # add the optimizer and loss function to the network
